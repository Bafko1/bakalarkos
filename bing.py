...
# Txt
train_text_path = os.path.join(base_path, "sample200.txt")
...
class AudioClassifier(nn.Module):
    def __init__(self, num_classes=2, input_channels=15):  # Adjust the number of input channels here
...
model = AudioClassifier(num_classes=2, input_channels=15).to(device)  # Adjust input_channels as needed
...
dev_dataset = AudioDataset(root_dirs=[dev_path], file_name=dev_text_path)  # Development dataset

train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, collate_fn=collate_fn)
dev_loader = DataLoader(dev_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn)  # Development loader
